...
export const Task2 = (): JSX.Element => {
    ...
    const [generatedValues, setGeneratedValues] = useState<boolean[][]>([]);
    ...
    const onGenerateValueClick = (): void => {
        if (generator) {
            setGeneratedValues(values => [...values, generator()]);
        }
    };

    return (
        <div className="flex flex-col w-full h-full gap-y-[10px]">
            <h2>Иммитация сложного события</h2>
            ...
            <button
                type="button"
                className="transition-all py-2.5 px-5 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
            ...
            <textarea
                ...
                value={generatedValues.map(item => JSON.stringify(item)).join('\n')}
            />
        </div>
    );
};
